...
# Definir zona horaria de México (CDMX) - Usando timezone nativo de Python
MEXICO_TZ = timezone(timedelta(hours=-6))  # UTC-6 (hora estándar de México)
        ...
        if teams.data:
            ...
            if st.button("⚽ AGREGAR PARTIDO", use_container_width=True, type="primary"):
                ...
                if home_team_id != away_team_id:
                    try:
                        # Crear datetime en zona horaria de México
                        match_datetime = datetime.combine(match_date, match_time)
                        match_datetime = match_datetime.replace(tzinfo=MEXICO_TZ)
                        data = {
