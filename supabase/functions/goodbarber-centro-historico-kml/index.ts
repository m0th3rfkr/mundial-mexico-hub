...
serve(async (req: Request) => {
  ...
  try {
    ...
    const CORREDOR_ID = '1a607716-bad4-4982-ba13-99884c02a730'
    ...
    const allPoints = [...(restaurantes.data || []).map(r => ({ ...r, category: 'restaurante', icon: '🍽️' })),...(hoteles.data || []).map(h => ({ ...h, category: 'hotel', icon: '🏨' })),...(imperdibles.data || []).map(i => ({ ...i, category: 'imperdible', icon: '🎯' })),...(estacionamientos.data || []).map(e => ({ ...e, category: 'estacionamiento', icon: '🅿️' })),...(ecobici.data || []).map(e => ({ ...e, category: 'ecobici', icon: '🚲' }))]
    const placemarks = allPoints.map((point: any) => {const lat = parseFloat(point.latitud); const lng = parseFloat(point.longitud); if (isNaN(lat) || isNaN(lng)) return ''; let description = ''; if (point.imagen_principal_url) description += `<img src="${escapeXml(point.imagen_principal_url)}" width="300" style="border-radius: 8px; margin-bottom: 10px;"/><br/>`; description += `<b>${point.icon} ${escapeXml(point.category)}</b><br/>`; if (point.descripcion_corta || point.descripcion) description += `<p>${escapeXml(point.descripcion_corta || point.descripcion)}</p><br/>`; if (point.direccion) description += `📍 ${escapeXml(point.direccion)}`; return `<Placemark><name>${escapeXml(point.nombre)}</name><description><![CDATA[${description}]]></description><Point><coordinates>${lng},${lat},0</coordinates></Point></Placemark>`}).filter(p => p !== '').join('\n')
    const kml = `<?xml version="1.0" encoding="UTF-8"?><kml xmlns="http://www.opengis.net/kml/2.2"><Document><name>Centro Histórico - CDMX</name><description>Todos los puntos de interés del corredor Centro Histórico</description>${placemarks}</Document></kml>`
